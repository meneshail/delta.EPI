...
result=read.table(RESULT_PATH,header = F,
                  col.names = c("chr1",'start1','end1','chr2','start2','end2','enhancer_name','gene_id','strand1','strand2','overlap1','overlap2','bin','sig','method'),sep = '\t')


result2= distinct(result,result$enhancer_name,result$gene_id,result$method,.keep_all = T)[,c(1:10,15)]
#result2$start1=apply(as.array(result2$start1),1,as.numeric)
#result2$end1=apply(as.array(result2$end1),1,as.numeric)
#result2$start2=apply(as.array(result2$start2),1,as.numeric)
#result2$end2=apply(as.array(result2$end2),1,as.numeric)
...
result2=result2[order(result2$chr1,result2$start1,result2$start2,result2$start2),]
...
print("Creating pnd for enrichment analysis...")
...
######################################################
#get GO terms
print("Getting GO terms for result genes...")
info=queryMany(my_genes, scopes='entrezgene', fields=c('name','entrezgene', 'go'), species=my_species)
#colnames(info)
info_simple=as.data.frame(info[,c(4,5)])
info_simple$BP=''
info_simple$CC=''
info_simple$MF=''
for (i in 1:nrow(info)){
  bp=unique(info$go.BP[[i]]$term)
  num=min(5,length(bp))
  info_simple[i,]$BP=paste(bp[1:num],sep = '',collapse = ' ; ')
  
  
  cc=unique(info$go.CC[[i]]$term)
  num=min(5,length(cc))
  info_simple[i,]$CC=paste(cc[1:num],sep = '',collapse = ' ; ')
  
  
  mf=unique(info$go.MF[[i]]$term)
  num=min(5,length(mf))
  info_simple[i,]$MF=paste(mf[1:num],sep = '',collapse = ' ; ')
}
result2=merge(result2,info_simple,by.x='gene_id',by.y='entrezgene')[,c(2:8,1,9:15)]
result2$Score='1'
